			...
			<?php
				...
				if ($res){
					...
					while($result = $consulta->fetch()){
						...
						print ("<tr><td><a href=\"atualizarL1.php?isbn=$isbn&
						titulo=$titulo&genero=$genero&precoT=$precoT&quantStock=$quantStock\">$isbn</td></a><td>$titulo</td><td>$genero</td>
						<td>$precoT €</td><td>$quantStock</td></tr>");
					}	
					print ("</table>");
					//Passar os registos por página
					if (($pag_ant) && ($pag>1)) {
						echo "<a class = \"tipo\" href=\"$php_self?pag=$pag_ant&isbn=$isbn&
						titulo=$titulo&genero=$genero&precoT=$precoT&quantStock=$quantStock&sql=$sql\">Anterior </a>|- ";
					}
					for ($i=1; $i<=$num_pag;$i++) {
						if($i !=$pag) {
							echo "<a class = \"tipo\" href=\"$php_self?pag=$i&isbn=$isbn&
						titulo=$titulo&genero=$genero&precoT=$precoT&quantStock=$quantStock&sql=$sql\">$i</a>-| ";
						}else {
							echo"$i -| ";
						}
					}
					if ($pag+1 < $num_pag) {
						echo  "<a class = \"tipo\" href=\"$php_self?pag=$pag_seg&isbn=$isbn&
						titulo=$titulo&genero=$genero&precoT=$precoT&quantStock=$quantStock&sql=$sql\"> Seguinte </a>";
					}
				}else{
