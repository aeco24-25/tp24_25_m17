			...
			<?php
				...
				if ($res){
					...
					while($result = $consulta->fetch()){
						...
						$email = $result['email'];
						print ("<tr><td><a href=\"atualizarE1.php?codE=$codE&
						nome=$nome&nif=$nif&morada=$morada&telefone=$telefone&email=$email\">$codE</td></a><td>$nome</td><td>$nif</td>
						<td>$morada</td><td>$telefone</td><td>$email</td></tr>");
					}	
					print ("</table>");
					//Passar os registos por página
					if (($pag_ant) && ($pag>1)) {
						echo "<a class = \"tipo\" href=\"$php_self?pag=$pag_ant&codE=$codE&
						nome=$nome&nif=$nif&morada=$morada&telefone=$telefone&email=$email&sql=$sql\">Anterior </a>|- ";
					}
					for ($i=1; $i<=$num_pag;$i++) {
						if($i !=$pag) {
							echo "<a class = \"tipo\" href=\"$php_self?pag=$i&codE=$codE&
						nome=$nome&nif=$nif&morada=$morada&telefone=$telefone&email=$email&sql=$sql\">$i</a>-| ";
						}else {
							echo"$i -| ";
						}
					}
					if ($pag+1 < $num_pag) {
						echo  "<a class = \"tipo\" href=\"$php_self?pag=$pag_seg&codE=$codE&
						nome=$nome&nif=$nif&morada=$morada&telefone=$telefone&email=$email&sql=$sql\"> Seguinte </a>";
					}
				}else{
