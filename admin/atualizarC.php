			...
			<?php
				...
				if ($res){
					...
					while($result = $consulta->fetch()){
						...
						print ("<tr><td><a href=\"atualizarC1.php?codCL=$codCL&
						nome=$nome&morada=$morada&telefone=$telefone\">$codCL</td></a><td>$nome</td><td>$morada</td>
						<td>$telefone</td></tr>");
					}	
					print ("</table>");
					//Passar os registos por página
					if (($pag_ant) && ($pag>1)) {
						echo "<a class = \"tipo\" href=\"$php_self?pag=$pag_ant&codCL=$codCL&
						nome=$nome&morada=$morada&telefone=$telefone&sql=$sql\">Anterior </a>|- ";
					}
					for ($i=1; $i<=$num_pag;$i++) {
						if($i !=$pag) {
							echo "<a class = \"tipo\" href=\"$php_self?pag=$i&codCL=$codCL&
						nome=$nome&morada=$morada&telefone=$telefone&sql=$sql\">$i</a>-| ";
						}else {
							echo"$i -| ";
						}
					}
