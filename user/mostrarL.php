		...
		<section class="col-6">
			<?php
				...
				if ($res){
					...
					print ("<table>");
					print ("<tr><th>Título</th><th>Género</th><th>Preço Tabela</th>
					...
					while($result = $consulta->fetch()){
						...
						print ("<tr><td>$titulo</td><td>$genero</td>
					...
					if (($pag_ant) && ($pag>1)) {
						...
						genero=$genero&precoT=$precoT&quantStock=$quantStock&sql=$sql\">Anterior </a>|- ";
					}
					for ($i=1; $i<=$num_pag;$i++) {
						if($i !=$pag) {
							echo "<a class = \"tipo\" href=\"$php_self?pag=$i&titulo=$titulo&
						genero=$genero&precoT=$precoT&quantStock=$quantStock&sql=$sql\">$i</a>-| ";
						}else {
							echo"$i -| ";
						}
					}
					if ($pag+1 < $num_pag) {
						...
						genero=$genero&precoT=$precoT&quantStock=$quantStock&sql=$sql\"> Seguinte </a>";
					}
				}else{
